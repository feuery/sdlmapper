...
(defmacro-export! defeffect (event-name param-list &rest lambda)
  `(setf *effects* (fset:with *effects* ,event-name (make-event 
						     (lambda ,param-list
						       ,@lambda)
						     nil
						     nil))))
...
(defeffect "PRINT-HELLOWORLD" (doc data-from-previous-event)
  ...
  (format t "hello from effect, here's no sprites: ~a~% " 'no)
...
(defun dispatcher ()
  ...
  (while *dispatcher-running*
    (if-let (event (pop *message-queue*))
      (progn (format t "Popping from *mq*~%")
	     (let ((new-doc (handle-event *engine-document* event)))
	       (if new-doc
		   (set-engine-doc new-doc)
		   (format t "Event ~a returned nil doc~%" event)))))
    (sleep 0.002)))
